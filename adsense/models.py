...
class LiencenceUser(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.key:  # Generate a key only if it doesn't exist already
            ...
            self.proxy.set(Proxy.objects.filter(general_proxy=True))
        else:
            super(LiencenceUser, self).save(*args, **kwargs)
...
class Proxy(models.Model):
    ...
    timezone = models.CharField(
        max_length=500, choices=TIMEZONE_CHOICES, default="US/Central"
    )
    proxy = models.TextField()
    general_proxy = models.BooleanField(default=False)

    def __str__(self) -> str:
        try:
            list_proxy = [
                item.strip() for item in self.proxy.split("\n") if item.strip()
            ]
            if len(list_proxy) > 0:
                return list_proxy[0]
            return self.proxy
...
class StaticFile(models.Model):
    file_name = models.CharField(
        max_length=100,
        choices=(("static_proxy", "static_proxy"), ("user_agent", "user_agent")),
        null=True,
        blank=True,
    )
    file = models.FileField(
        upload_to="static/adsense_exe_files/", storage=OverwriteStorage()
    )
...
class UserAgent(models.Model):
    ...
    browser_string = models.CharField(
        default="Version/{version}.0 Mobile Safari/537.36"
    )
...
class AdsenseLog(models.Model):
    ...
    def __str__(self) -> str:
        return f"{self.ip} -- {self.website}"
    
    def save(self, *args, **kwargs):
        if self.pk:  # If the instance has a primary key, it's being updated
            self.updated_at = timezone.now()  # Update updated_at only when updating
